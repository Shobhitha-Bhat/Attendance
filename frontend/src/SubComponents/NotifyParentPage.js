...
const [msg,setmsg]=useState('')
  ...
  const sendMsgDetails=async(e)=>{
    e.preventDefault();
    try{
      await Axios.post("http://localhost:8000/addnewparentmessage",{
        facultyMail,parentMail,studentUsn,phone,subject,msg
      })
    ...
    <div className="container mt-4">
      ...
      <Form className="p-3 border rounded bg-white" onSubmit={sendMsgDetails}>
        ...
        <Form.Group className="mb-3" controlId="formFrom">
          ...
          <Form.Control type="email" value={facultyMail} onChange={(e)=>setfacultyMail(e.target.value)} placeholder="Enter Faculty Mail" />
        </Form.Group>

        <Form.Group className="mb-3" controlId="formTo">
          <Form.Label>To</Form.Label>
          <Form.Control type="email" value={parentMail} onChange={(e)=>setparentMail(e.target.value)} placeholder="Enter Parent recipient email" />
        </Form.Group>
