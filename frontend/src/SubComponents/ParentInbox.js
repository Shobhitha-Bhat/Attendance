...
export const ParentInbox = () => {
  ...
  const {parameter='Parent - Inbox'}=location.state || {};
    const[messages,setmessages]=useState([])
   
    const getMessages=async()=>{
        const response=await Axios.get("http://localhost:8000/getparentmessages")
...
getMessages()
  ...
  <h1 className='text-center mb-5 mt-5'>{parameter}</h1>
  <div>{
    messages.length===0 ? (
